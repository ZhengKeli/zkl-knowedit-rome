...
class LogitLens:
    ...
    def __init__(
        ...
        self.lm_head = get_module(model, lm_head_module)
        self.ln_f = get_module(model, ln_f_module)

        self.output: Optional[dict] = None
        self.td: Optional[TraceDict] = None
        self.trace_layers = [layer_module_tmp.format(layer) for layer in range(self.n_layers)]
    ...
    def __exit__(self, *args):
        ...
        with torch.no_grad():
            for layer, (_, t) in enumerate(self.td.items()):
                cur_out = t.output[0]
                assert (cur_out.size(0) == 1), "Make sure you're only running LogitLens on single generations only."
                self.output[layer] = torch.softmax(self.lm_head(self.ln_f(cur_out[:, -1, :])), dim=1)
    ...
    def pprint(self, k=5):
        ...
        for layer, pred in self.output.items():
            ...
            for i in range(k):
                to_print[layer].append((self.tok.decode(rets[1][i]), round(rets[0][i].item() * 1e2) / 1e2,))
